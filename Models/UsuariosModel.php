<?php 
	...
	class UsuariosModel extends Mysql
	{
		...
		public function selectUsuario(int $idpersona){
			...
			$sql = "SELECT p.idpersona,p.identificacion,p.nombres,p.apellidos,p.telefono,p.email_user,p.whatsapp,p.fecha_nac,p.direccionuser,r.idrol,r.nombrerol,p.status, DATE_FORMAT(p.datecreated, '%d-%m-%Y') as fechaRegistro 
					FROM persona p
					INNER JOIN rol r
					ON p.rolid = r.idrol
		...
		public function deleteUsuario1(int $intIdpersona)
		{
			$this->intIdUsuario = $intIdpersona;
		...
		public function updateDataFiscal(int $idUsuario, string $strwhatsapp, string $strNomFiscal, string $strDirFiscal){
			$this->intIdUsuario = $idUsuario;
			$this->strNit = $strNit;
			$this->strNomFiscal = $strNomFiscal;
			$this->strDirFiscal = $strDirFiscal;
			$sql = "UPDATE persona SET whatsapp=?, fecha_nac=?, direccionuser=? 
						WHERE idpersona = $this->intIdUsuario ";
			$arrData = array($this->strwhatsapp,
							$this->strNomFiscal,
							$this->strDirFiscal);
			$request = $this->update($sql,$arrData);
		    return $request;
